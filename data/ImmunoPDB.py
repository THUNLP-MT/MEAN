...
def Chain_switch_numbering_scheme(self, scheme, domain_index=0):
    ...
    if 'anarci_details' in self.xtra: # Only bother to recurse into numbered chains 
        ...
        children = list(self)
        for child in children:  # editing id to an existed id raise error in newer biopython version, so detach first
            self.detach_child(child.get_id())
        for child in children:  
            child.switch_numbering_scheme(scheme, domain_index=domain_index)
        # self.child_dict = {} # Reset the child dict on a separate loop to prevent conflicts
        for child in children:
            if child.get_id() in self.child_dict:  # redundant residue
                continue
            self.add(child)
...
def pairwise_muscle(seq1, seq2,exact=True):
    ...
    if exact:
        p = subprocess.Popen( [muscle_path,'-gapopen', '-1.001','-gapextend', '-1'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True )
    else:
        p = subprocess.Popen( [muscle_path], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True )
