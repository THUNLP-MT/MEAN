...
# from .encoder import MPNEncoder
...
class HierarchicalDecoder(nn.Module):
    ...
    def generate(self, true_S, true_cdr, mask, return_ppl=False):
        ...
        S = S.tolist()
        S = [''.join([VOCAB.idx_to_symbol(S[i][j]) for j in range(cdr_range[i][0], cdr_range[i][1] + 1)]) for i in range(B)]
    ...
    def infer(self, batch, device):
        ...
        seqs, ppls, xs = self.generate(true_S, true_L, mask, return_ppl=True)
        true_X = batch['X'].numpy()

        xs = xs.detach().cpu().numpy()

        res_seqs, res_xs, res_true_xs = [], [], []
        for i in range(len(seqs)):
            ...
            assert r - l + 1 == len(seq)
            true_x = true_X[i][l:r+1]  # [N, 4, 3]
            res_true_xs.append(true_x)
        return ppls.detach().cpu().numpy().tolist(), res_seqs, res_xs, res_true_xs, False